...
void node_label::emit( program_t *p )
{
  ...
  p->instructions.emplace_back( new instruction_t{} );
  if( left ){ left->emit(p); }
  if( return_ ){ p->instructions.emplace_back( new instruction_return_t{} ); }
}

void node_goto::emit( program_t *p )
{
  p->instructions.emplace_back( new instruction_goto_t{data,push} );
}
void node_return::emit( program_t *p )
{
  p->instructions.emplace_back( new instruction_return_t{} );
}
